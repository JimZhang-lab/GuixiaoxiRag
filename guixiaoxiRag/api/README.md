# GuiXiaoXiRag Server and WebUI

The GuiXiaoXiRag Server is designed to provide a Web UI and API support. The Web UI facilitates document indexing, knowledge graph exploration, and a simple RAG query interface. GuiXiaoXiRag Server also provides an Ollama-compatible interface, aiming to emulate GuiXiaoXiRag as an Ollama chat model. This allows AI chat bots, such as Open WebUI, to access GuiXiaoXiRag easily.
...
```bash
pip install "guixiaoxiRag-hku[api]"
```
...
git clone https://github.com/HKUDS/guixiaoxiRag.git
...
### Before Starting GuiXiaoXiRag Server

GuiXiaoXiRag necessitates the integration of both an LLM (Large Language Model) and an Embedding Model to effectively execute document indexing and querying operations. Prior to the initial deployment of the GuiXiaoXiRag server, it is essential to configure the settings for both the LLM and the Embedding Model. GuiXiaoXiRag supports binding to various LLM/Embedding backends:
...
It is recommended to use environment variables to configure the GuiXiaoXiRag Server. There is an example environment variable file named `env.example` in the root directory of the project. Please copy this file to the startup directory and rename it to `.env`. After that, you can modify the parameters related to the LLM and Embedding models in the `.env` file. It is important to note that the GuiXiaoXiRag Server will load the environment variables from `.env` into the system environment variables each time it starts. **GuiXiaoXiRag Server will prioritize the settings in the system environment variables to .env file**.
...
### Starting GuiXiaoXiRag Server

The GuiXiaoXiRag Server supports two operational modes:
...
```
guixiaoxiRag-gunicorn --workers 4
```

When starting GuiXiaoXiRag, the current working directory must contain the `.env` configuration file. **It is intentionally designed that the `.env` file must be placed in the startup directory**. The purpose of this is to allow users to launch multiple GuiXiaoXiRag instances simultaneously and configure different `.env` files for different instances. **After modifying the `.env` file, you need to reopen the terminal for the new settings to take effect.** This is because each time GuiXiaoXiRag Server starts, it loads the environment variables from the `.env` file into the system environment variables, and system environment variables have higher precedence.
...
- `--workspace`: Workspace name, used to logically isolate data between multiple GuiXiaoXiRag instances (default: empty)

### Launching GuiXiaoXiRag Server with Docker
...
services:
  guixiaoxiRag:
    ...
    image: ghcr.io/hkuds/guixiaoxiRag:latest
    ports:
      - "${PORT:-9621}:9621"
    volumes:
...
* Start the GuiXiaoXiRag Server with the following command:
...
> You can get the official docker compose file from here: [docker-compose.yml](https://raw.githubusercontent.com/HKUDS/GuiXiaoXiRag/refs/heads/main/docker-compose.yml). For historical versions of GuiXiaoXiRag docker images, visit this link: [GuiXiaoXiRag Docker Images](https://github.com/HKUDS/GuiXiaoXiRag/pkgs/container/guixiaoxiRag)

### Auto scan on startup

When starting the GuiXiaoXiRag Server with the `--auto-scan-at-startup` parameter, the system will automatically:
...
### Starting Multiple GuiXiaoXiRag Instances

There are two ways to start multiple GuiXiaoXiRag instances. The first way is to configure a completely independent working environment for each instance. This requires creating a separate working directory for each instance and placing a dedicated `.env` configuration file in that directory. The server listening ports in the configuration files of different instances cannot be the same. Then, you can start the service by running `guixiaoxiRag-server` in the working directory.

The second way is for all instances to share the same set of `.env` configuration files, and then use command-line arguments to specify different server listening ports and workspaces for each instance. You can start multiple GuiXiaoXiRag instances in the same working directory with different command-line arguments. For example:

```
# Start instance 1
guixiaoxiRag-server --port 9621 --workspace space1

# Start instance 2
guixiaoxiRag-server --port 9622 --workspace space2
```
...
### Install GuiXiaoXiRag as a Linux Service

Create your service file `guixiaoxiRag.service` from the sample file: `guixiaoxiRag.service.example`. Modify the `WorkingDirectory` and `ExecStart` in the service file:

```text
Description=GuiXiaoXiRag Ollama Service
WorkingDirectory=<guixiaoxiRag installed directory>
ExecStart=<guixiaoxiRag installed directory>/guixiaoxiRag/api/guixiaoxiRag-api
```

Modify your service startup script: `guixiaoxiRag-api`. Change your Python virtual environment activation command as needed:

```shell
#!/bin/bash

# your python virtual environment activation
source /home/netman/guixiaoxiRag-xyj/venv/bin/activate
# start guixiaoxiRag api server
guixiaoxiRag-server
```

Install GuiXiaoXiRag service. If your system is Ubuntu, the following commands will work:

```shell
sudo cp guixiaoxiRag.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl start guixiaoxiRag.service
sudo systemctl status guixiaoxiRag.service
sudo systemctl enable guixiaoxiRag.service
```

## Ollama Emulation

We provide Ollama-compatible interfaces for GuiXiaoXiRag, aiming to emulate GuiXiaoXiRag as an Ollama chat model. This allows AI chat frontends supporting Ollama, such as Open WebUI, to access GuiXiaoXiRag easily.

### Connect Open WebUI to GuiXiaoXiRag

After starting the guixiaoxiRag-server, you can add an Ollama-type connection in the Open WebUI admin panel. And then a model named `guixiaoxiRag:latest` will appear in Open WebUI's model management interface. Users can then send queries to GuiXiaoXiRag through the chat interface. You should install GuiXiaoXiRag as a service for this use case.
...
The default query mode is `hybrid` if you send a message (query) from the Ollama interface of GuiXiaoXiRag. You can select query mode by sending a message with a query prefix.

A query prefix in the query string can determine which GuiXiaoXiRag query mode is used to generate the response for the query. The supported prefixes include:
...
For example, the chat message `/mix What's GuiXiaoXiRag?` will trigger a mix mode query for GuiXiaoXiRag. A chat message without a query prefix will trigger a hybrid mode query by default.

`/bypass` is not a GuiXiaoXiRag query mode; it will tell the API Server to pass the query directly to the underlying LLM, including the chat history. So the user can use the LLM to answer questions based on the chat history. If you are using Open WebUI as a front end, you can just switch the model to a normal LLM instead of using the `/bypass` prefix.

`/context` is also not a GuiXiaoXiRag query mode; it will tell GuiXiaoXiRag to return only the context information prepared for the LLM. You can check the context if it's what you want, or process the context by yourself.

### Add user prompt in chat

When using GuiXiaoXiRag for content queries, avoid combining the search process with unrelated output processing, as this significantly impacts query effectiveness. User prompt is specifically designed to address this issue — it does not participate in the RAG retrieval phase, but rather guides the LLM on how to process the retrieved results after the query is completed. We can append square brackets to the query prefix to provide the LLM with the user prompt:
...
By default, the GuiXiaoXiRag Server can be accessed without any authentication. We can configure the server with an API Key or account credentials to secure it.
...
GuiXiaoXiRag API Server implements JWT-based authentication using the HS256 algorithm. To enable secure access control, the following environment variables are required:
...
RESOURCE_GROUP_NAME=GuiXiaoXiRag
LOCATION=swedencentral
RESOURCE_NAME=GuiXiaoXiRag-OpenAI
...
## GuiXiaoXiRag Server Configuration in Detail
...
GuiXiaoXiRag supports binding to various LLM/Embedding backends:
...
GuiXiaoXiRag uses 4 types of storage for different purposes:
...
```
Example connection configurations for each storage type can be found in the `env.example` file. The database instance in the connection string needs to be created by you on the database server beforehand. GuiXiaoXiRag is only responsible for creating tables within the database instance, not for creating the database instance itself. If using Redis as storage, remember to configure automatic data persistence rules for Redis, otherwise data will be lost after the Redis service restarts. If using PostgreSQL, it is recommended to use version 16.6 or above.
...
You cannot change storage implementation selection after adding documents to GuiXiaoXiRag. Data migration from one storage implementation to another is not supported yet. For further information, please read the sample env file or config.ini file.

### GuiXiaoXiRag API Server Command Line Options
...
| --key                 | None          | API key for authentication. Protects the GuiXiaoXiRag server against unauthorized access                                            |
...
### For JWT Auth
# AUTH_ACCOUNTS='admin:admin123,user1:pass456'
# TOKEN_SECRET=your-key-for-GuiXiaoXiRag-API-Server-xxx
...
GuiXiaoXiRag implements asynchronous document indexing to enable frontend monitoring and querying of document processing progress. Upon uploading files or inserting text through designated endpoints, a unique Track ID is returned to facilitate real-time progress monitoring.
