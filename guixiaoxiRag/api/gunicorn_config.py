...
import os
import logging
from guixiaoxiRag.kg.shared_storage import finalize_share_data
from guixiaoxiRag.utils import setup_logger, get_env_value
...
errorlog = os.getenv("ERROR_LOG", log_file_path)  # Default write to guixiaoxiRag.log
accesslog = os.getenv("ACCESS_LOG", log_file_path)  # Default write to guixiaoxiRag.log
...
def on_starting(server):
    ...
    # Log the location of the GuiXiaoXiRag log file
    print(f"GuiXiaoXiRag log file: {log_file_path}\n")
...
def post_fork(server, worker):
    ...
    setup_logger("guixiaoxiRag", log_level, add_filter=True, log_file_path=log_file_path)

    # Set up guixiaoxiRag submodule loggers
    for name in logging.root.manager.loggerDict:
        if name.startswith("guixiaoxiRag."):
